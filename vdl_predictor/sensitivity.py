...
def main(args):
    ...
    step = 2. / (args.step - 1)
    sparams_sweep = Variable(torch.zeros(1, 3), requires_grad=True)

    grads = np.zeros(args.step)
    for i in tqdm(range(args.step)):
        if args.param_idx == 0:
            sparams_sweep[0, 0] = -1. + i * step
            sparams_sweep[0, 1] = ((args.omb - OmBMin) / (OmBMax - OmBMin)) * 2. - 1.
            sparams_sweep[0, 2] = ((args.h - hMin) / (hMax - hMin)) * 2. - 1.
        elif args.param_idx == 1:
            sparams_sweep[0, 0] = ((args.omm - OmMMin) / (OmMMax - OmMMin)) * 2. - 1.
